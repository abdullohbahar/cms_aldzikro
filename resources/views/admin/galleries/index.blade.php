...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.galleries.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-sm transition flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition group">
        <div class="relative h-48 bg-gray-100">
            ...
            <div class="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition duration-300">
                <a href="{{ route('admin.galleries.edit', $gallery) }}" class="bg-white p-2 rounded-lg shadow text-blue-600 hover:text-blue-700" title="Edit">
                    <i class="fas fa-edit"></i>
                </a>
                <form action="{{ route('admin.galleries.destroy', $gallery) }}" method="POST" class="inline" onsubmit="confirmDelete(event)">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-white p-2 rounded-lg shadow text-red-600 hover:text-red-700" title="Hapus">
        ...
        <div class="p-5">
            ...
            <p class="text-sm text-gray-500 mb-4 line-clamp-2">{{ $gallery->description ?? 'Tidak ada deskripsi' }}</p>
            ...
            <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                ...
                <a href="{{ route('admin.galleries.show', $gallery) }}" class="text-sm font-medium text-blue-600 hover:text-blue-500 flex items-center">
