...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6 mt-8">
        ...
        <a href="{{ route('admin.galleries.create', ['parent_id' => $gallery->id]) }}"
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @forelse($gallery->children as $item)
            <div
                class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition">
                <div class="relative h-48 bg-gray-100 cursor-pointer overflow-hidden">
                    ...
                    <div
                        class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100 gap-2">
                        <a href="{{ route('admin.galleries.edit', $item) }}"
                            class="bg-white text-blue-600 p-2 rounded-full shadow hover:bg-gray-100 transition"
                            title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('admin.galleries.destroy', $item) }}" method="POST" class="inline"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="bg-white text-red-600 p-2 rounded-full shadow hover:bg-gray-100 transition"
                                title="Hapus">
