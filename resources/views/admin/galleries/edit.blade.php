        ...
        <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="parent_id" id="parent_id" 
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">Buat sebagai Album</option>
                    @foreach($albums as $album)
                        <option value="{{ $album->id }}" {{ old('parent_id', $gallery->parent_id) == $album->id ? 'selected' : '' }}>
                            {{ $album->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" 
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $gallery->description) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center justify-center w-full">
                    <label for="image-upload" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 overflow-hidden relative">
                        <!-- Placeholder (Hidden if has image) -->
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 {{ $gallery->image_path ? 'hidden' : '' }}" id="upload-placeholder">
                        ...
                        <!-- Preview (Shown if has image or uploaded) -->
                        <img id="image-preview" src="{{ $gallery->image_path ? asset('storage/' . $gallery->image_path) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $gallery->image_path ? '' : 'hidden' }}">
